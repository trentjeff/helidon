...
public class CdiExtension implements Extension {
    ...
    private static final String BEAN_PREFIX = "helidon-inject-";

    private final LazyValue<InjectionServices> injectionServices =
            LazyValue.create(() -> InjectionServices.injectionServices().orElseThrow());
...
//     * @throws IllegalStateException when invoked when server (and hence the context) is not started
//     */
...
//    /**
//     * Load all service/bean definitions and register our interceptor binding, so we can
//     * execute interceptors using a CDI interceptor.
//     *
...
//    }
    ...
    /**
     * Add all Helidon Inject Service/beans for injection.
     *
     * @param event CDI event
     */
...
//                .id(BEAN_PREFIX  + "context")
//                .scope(ApplicationScoped.class)
//                .produceWith(instance -> micronautContext.get());

        // add the services / beans from registry
        ServiceInfoCriteria allCriteria = ServiceInfoCriteria.builder().build();
        List<ServiceProvider<?>> allServiceProviders =
                InjectionServices.realizedServices().lookupAll(allCriteria);
        for (ServiceProvider<?> sp : allServiceProviders) {
            Class<?> beanType = sp.serviceType();
...
//            }
            ...
            event.addBean()
                    .addType(beanType)
                    .id(BEAN_PREFIX + beanType.getName())
                    // inject using dependent - manage scope by helidon injection context
                    .scope(Dependent.class)
                    .qualifiers(toAnnotations(serviceInfo.qualifiers()))
                    .priority((int) serviceInfo.realizedWeight())
                    .produceWith(instance -> sp.get());
...
//        unprocessedBeans.clear();
    ...
    private static Set<Annotation> toAnnotations(Set<Qualifier> qualifiers) {
        return qualifiers.stream()
                .map(AnnotationSupport::toAnnotation)
                .collect(Collectors.toSet());
    }

    //    /**
//     * Start application context (and make it available for other extensions).
//     *
//     * @param event CDI event
//     */
...
//            context.close();
//        }
        InjectionServices.injectionServices().orElseThrow().shutdown();
    }
